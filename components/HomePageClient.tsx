...
export function HomePageClient({ initialData }: HomePageClientProps): React.JSX.Element {
  const { language, isReady, hasSelection, setLanguagePreference } = useLanguagePreference();
  const [dataset, setDataset] = useState<ElectionDataset>(initialData);
